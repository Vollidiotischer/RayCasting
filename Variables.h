...
struct Player {
  ...
  Player(int x, int y, int r, float speed = 0.05)
  ...
  void update_position() { shape.setPosition(x, y); }
};
...
struct Obstacle {
  ...
  Obstacle(int x, int y, int w, int h, bool is_rect = true,
           bool is_in_creation = true)
      : x(x), y(y), w(w), h(h), is_rect(is_rect),
        is_in_creation(is_in_creation) {}
};
...
struct Line {
  ...
  Line(int x, int y, int w, int h, int thickness, float angle = 0)
      : x(x), y(y), w(w), h(h), thickness(thickness), angle(angle) {

    this->length = sqrt((float)(w * w + h * h));
  }

  float calculate_angle() { return acos(w / length) * 180.0 / 3.1415; }
};