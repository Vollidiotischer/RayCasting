...
void update_obstacles_in_creation(sf::RenderWindow &rw,
...
void calculate_ray_length(std::vector<Obstacle> &obstacles, Player &player) {
  ...
  Calculate Intersection between the ray and each side of an obstacle
          ...
          -> lambda_rx_2 => if the ray hits the side (and the intersection is
  not outside of [0, 1])
  ...
  for (int i = 0; i < player.rays.size(); i++) {
    ...
    for (int i2 = 0; i2 < obstacles.size(); i2++) {
      ...
      if (obstacles[i2].is_rect) {
        ...
        // Ray = R1 [Top]
        float lambda_r1_1 =
            (obstacles[i2].y - player.rays[i].y) / player.rays[i].dy;
        float lambda_r1_2 = (player.rays[i].x - obstacles[i2].x +
                             lambda_r1_1 * player.rays[i].dx) /
                            obstacles[i2].w;
        lambda_r1_1 =
            lambda_r1_2 >= 0 && lambda_r1_2 <= 1.0 ? lambda_r1_1 : 1.0;

        // Ray = R2 [Left]
        float lambda_r2_1 =
            (obstacles[i2].x - player.rays[i].x) / player.rays[i].dx;
        float lambda_r2_2 = (player.rays[i].y - obstacles[i2].y +
                             lambda_r2_1 * player.rays[i].dy) /
                            obstacles[i2].h;
        lambda_r2_1 =
            lambda_r2_2 >= 0 && lambda_r2_2 <= 1.0 ? lambda_r2_1 : 1.0;

        // Ray = R3 [Right]
        float lambda_r3_1 =
            (obstacles[i2].x + obstacles[i2].w - player.rays[i].x) /
            player.rays[i].dx;
        float lambda_r3_2 = (player.rays[i].y - obstacles[i2].y +
                             lambda_r3_1 * player.rays[i].dy) /
                            obstacles[i2].h;
        lambda_r3_1 =
            lambda_r3_2 >= 0 && lambda_r3_2 <= 1.0 ? lambda_r3_1 : 1.0;

        // Ray = R4 [Bottom]
        float lambda_r4_1 =
            (obstacles[i2].y + obstacles[i2].h - player.rays[i].y) /
            player.rays[i].dy;
        float lambda_r4_2 = (player.rays[i].x - obstacles[i2].x +
                             lambda_r4_1 * player.rays[i].dx) /
                            obstacles[i2].w;
        lambda_r4_1 =
            lambda_r4_2 >= 0 && lambda_r4_2 <= 1.0 ? lambda_r4_1 : 1.0;

        // get the smallest lambda -> first obstacle hit
        float lambdas[] = {lambda_r1_1, lambda_r2_1, lambda_r3_1, lambda_r4_1,
                           min_lambda};
        min_lambda = find_min_lambda_range(lambdas);
      } else {

        // float lambda_1 = (o.h * o.x - o.y * o.w - o.h * r.x + o.w * r.y) /
        // (r.dx * o.h - r.dy * o.w);	  //	[WWW]
        float lambda_2 = (r.dy * o.x - r.dx * o.y - r.dy * r.x + r.dx * r.y) /
                         (r.dx * o.h - r.dy * o.w); //	[WWW]

        // float lambda_2 = ((o.h - r.y) * r.dx + (r.x - o.x) * r.dy) / (o.w *
        // r.dy - o.h * r.dx);

        float lambda_1 = (o.x - r.x + lambda_2 * o.w) / r.dx;

        if (lambda_2 >= 0 && lambda_2 <= 1 && lambda_1 < min_lambda &&
            lambda_1 >= 0) {
          min_lambda = lambda_1;
...
void calculate_frame(sf::RenderWindow &rw, Player &player,
                     std::vector<Obstacle> &obstacles) {
